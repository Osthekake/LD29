...
var Keyboard = {
    ...
    trigger : function(actor){
    	//enter
    	if(actor != null && this.keys[13] && actor.enter){
    		actor.enter();
    		this.keys[13] = 0;
		}
    	//left
    	if(actor != null && this.keys[37] && actor.left){
    		actor.left();
    		this.keys[37] = 0;
		}
    	//up
    	if(actor != null && this.keys[38] && actor.up){
    		actor.up();
    		this.keys[38] = 0;
		}
    	//right
    	if(actor != null && this.keys[39] && actor.right){
    		actor.right();
    		this.keys[39] = 0;
 		}
    	//down
    	if(actor != null && this.keys[40] && actor.down){
    		actor.down();
    		this.keys[40] = 0;
    	}
    }
